/**
 * Fix script to replace the main resolvers file and restart the API gateway
 */
...
// Content for the new resolvers file
...
const resolvers = {
  ...
  // Other type resolvers same as before...
};

module.exports = resolvers;
`;

// Backup the current resolvers file
const resolversPath = path.join(__dirname, 'resolvers.js');
const backupPath = path.join(__dirname, 'resolvers.backup.js');

async function fixResolvers() {
  try {
    console.log('[1/4] Backing up existing resolvers file...');
    if (fs.existsSync(resolversPath)) {
      fs.copyFileSync(resolversPath, backupPath);
      console.log(`  Backup created at: ${backupPath}`);
    }
    
    console.log('[2/4] Writing new resolvers file...');
    fs.writeFileSync(resolversPath, newResolversContent, 'utf8');
    
    console.log('[3/4] Stopping API gateway...');
    // Find and kill the gateway process
    const findProcess = spawn('powershell', ['Get-Process -Name "node" | Where-Object {$_.CommandLine -like "*api-gateway/server.js*"} | Stop-Process -Force']);
    
    findProcess.stdout.on('data', (data) => {
      console.log(`Process finder output: ${data}`);
    });
    
    findProcess.stderr.on('data', (data) => {
      console.error(`Process finder error: ${data}`);
    });
...
function startGateway() {
  console.log('[4/4] Starting API gateway with new resolvers...');
  ...
  console.log('API gateway should be restarting. Waiting a few seconds...');
  setTimeout(() => {
    console.log('API gateway has been updated with new resolvers!');
    console.log('You can now test your GraphQL queries again.');
  }, 3000);
}
